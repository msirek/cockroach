...
func (c *CustomFuncs) CanUseUniqueChecksForInsertFastPath(
	...
	for i := range ins.UniqueChecks {
		...
		// of any projections so we can inspect the scan or lookup join expressions
		// underneath.
		...
		lookupJoin, _ := check.(*memo.LookupJoinExpr)
		...
		if len(lookupJoin.LookupExpr) == 0 {
			...
			for i, keyCol := range lookupJoin.KeyCols {
				...
				out.InsertCols[i] = opt.TableColumnOrdinal(inputColOrd)
			}
		...
		for _, onFilter := range lookupJoin.On {
			if !c.ignorableNECheck(onFilter.Condition, withScan, lookupJoin.Table) {
				return memo.UniqueChecksExpr{}, nil, false
			}
		}
