...
func (c *CustomFuncs) CanUseUniqueChecksForInsertFastPath(
	...
	for i := range ins.UniqueChecks {
		...
		for ; expr != nil && expr.Op() == opt.LookupJoinOp; expr = expr.NextExpr() {
			...
			referencedTable := md.Table(lookupJoin.Table)
			if _, isPartial := referencedTable.Index(lookupJoin.Index).Predicate(); isPartial {
...
func (c *CustomFuncs) handleSingleRowInsert(
	...
	for check = check.FirstExpr(); check != nil; check = check.NextExpr() {
		...
		isPartialIndexScan := func(candidateScan *memo.ScanExpr) bool {
			referencedTable := md.Table(candidateScan.Table)
			if _, isPartial := referencedTable.Index(candidateScan.Index).Predicate(); isPartial {
				return true
			}
			return false
		}
		...
		for ; possibleScanOrSelect != nil; possibleScanOrSelect = possibleScanOrSelect.NextExpr() {
			...
			if scan, isScan := expr.(*memo.ScanExpr); isScan {
				...
				if scan.Constraint != nil && !isPartialIndexScan(scan) {
					rightExpr = scan
					break
				}
			}
