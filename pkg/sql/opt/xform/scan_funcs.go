...
func (c *CustomFuncs) GenerateIndexScans(grp memo.RelExpr, scanPrivate *memo.ScanPrivate) {
	...
	iter.ForEach(func(index cat.Index, filters memo.FiltersExpr, indexCols opt.ColSet, isCovering bool, constProj memo.ProjectionsExpr) {
		...
		// AllowNonCoveringIndexScan allows all index access paths to be explored,
		// even when non-covering, without forcing a particular index.
		if !scanPrivate.Flags.ForceIndex && !scanPrivate.Flags.AllowNonCoveringIndexScan {
			return
		}
