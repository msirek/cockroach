...
// NonCoveringIndexScanClusterMode controls the cluster setting for enabling
// non-covering index scans in the optimizer.
var NonCoveringIndexScanClusterMode = settings.RegisterBoolSetting(
	settings.TenantWritable,
	"sql.optimizer.non_covering_index_scan.enabled",
	"if enabled, non-covering index scan access paths are explored by the optimizer",
	false,
).WithPublic()
...
func (b *Builder) buildScan(
	...
	private := memo.ScanPrivate{Table: tabID, Cols: scanColIDs}
	if NonCoveringIndexScanClusterMode.Get(&b.evalCtx.Settings.SV) {
		private.Flags.AllowNonCoveringIndexScan = true
	}
	if indexFlags != nil {
